...
本仓库基于开源项目 RedRunner 进行学习型二次开发，目标是将其整理为游戏客户端应届生作品集项目。当前改造重点包括：阅读项目结构、修复运行时问题、新增 Dash 冲刺、二段跳、跳跃手感优化、动态难度、本地保存、数据 UI、成就提示、操作引导和对象池复用状态优化，并持续补充开发文档。
...
项目里已经有 `ObjectPool`，用于复用金币等可收集物，减少频繁 `Instantiate` 和 `Destroy` 带来的运行时开销。但是金币被收集后会关闭 `SpriteRenderer` 和 `Collider2D`，如果对象池后续再次取出同一个金币对象，而没有恢复这些状态，就可能出现“对象被生成了，但看不见、碰不到”的复用问题。

### 技术实现

本次优化给对象池补充了生命周期入口：

- 在 `IPoolable` 中新增 `OnSpawnFromPool()`
- 在 `Collectable` 中提供默认空实现，避免所有子类都必须重复写空方法
- 在 `ObjectPool.SpawnFromPool()` 激活对象并设置位置后，调用 `objectToSpawn.OnSpawnFromPool()`
- 在 `Coin.OnSpawnFromPool()` 中恢复金币的显示、碰撞、粒子和动画状态
- 在 `CoinRigidbody2D.OnSpawnFromPool()` 中额外恢复第二碰撞体，并清理刚体速度和角速度
...
面试时可以这样解释：我没有只关注功能新增，也检查了对象池复用时的状态残留问题。金币被收集后会关闭渲染和碰撞，因此重新从池里取出时必须重置状态，否则对象池反而会引入隐藏 bug。这个改动体现了我对对象生命周期、性能优化和运行时状态一致性的理解。
## 当前项目流程理解
...
为 IPoolable 增加 OnSpawnFromPool() 生命周期方法，并在金币重新从对象池生成时恢复显示、碰撞、粒子、动画和刚体状态，避免对象池复用后出现不可见或不可收集的问题。
